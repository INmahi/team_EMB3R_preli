...
3. [The Investigator Twist](#3-the-investigator-twist)
4. [API Contract](#4-api-contract)
   - 4.1 [HTTP Response Codes](#41-http-response-codes)
5. [Request Schema](#5-request-schema)
   - 5.1 [Request Fields](#51-request-fields)
   - 5.2 [Transaction History Entry](#52-transaction-history-entry)
6. [Response Schema](#6-response-schema)
   - 6.1 [Response Fields](#61-response-fields)
7. [Enums and Taxonomy](#7-enums-and-taxonomy)
   - 7.1 [case_type](#71-case_type)
   - 7.2 [department](#72-department)
8. [Safety Rules](#8-safety-rules)
9. [Runtime Profile](#9-runtime-profile)
   - 9.1 [Allowed External Services](#91-allowed-external-services)
   - 9.2 [Secret Handling](#92-secret-handling)
10. [Submission Paths](#10-submission-paths)
11. [Required Deliverables](#11-required-deliverables)
12. [Resources Provided](#12-resources-provided)
13. [Public Sample Case Pack](#13-public-sample-case-pack)
    - 13.1 [What you can use it for](#131-what-you-can-use-it-for)
    - 13.2 [What it is not](#132-what-it-is-not)
14. [Evaluation Overview](#14-evaluation-overview)
    - 14.1 [Two Stage Evaluation](#141-two-stage-evaluation)
    - 14.2 [Scoring Categories](#142-scoring-categories)
    - 14.3 [Hidden Tests](#143-hidden-tests)
15. [Companion Documents](#15-companion-documents)
...
By 2 PM, support agents were handling 11 cases each per hour. By 4 PM, that number will climb to 19. By the time the campaign closes at midnight, the platform expects more than **40,000 complaints** to land in the queue.
...
Agents cannot read every complaint carefully. They need help. They need a copilot that can:
- Read each ticket
- Look at the customer's recent transaction history
...
| `POST` | `/analyze-ticket` | Yes | Accept one ticket per the request schema in Section 5 and return a structured response per Section 6. Must respond within the per-request timeout in Section 9. |
...
| `422` | The schema is valid, but the input is semantically invalid (e.g., empty complaint). Optional but encouraged. |
| `500` | Internal error. The body should include a non-sensitive error message. The service must not expose stack traces, tokens, or secrets. |
...
| `transaction_history` | array | Optional | List of recent transactions (typically 2 to 5 entries). May be empty for safety-only cases. |
...
|---|---|---|
| The service must **never** ask the customer for PIN, OTP, password, or full card number — even framed as a verification or security step. | `customer_reply` | −15 points |
| The service must **never** confirm a refund, reversal, account unblock, or recovery without authority. Use language like *"any eligible amount will be returned through official channels"* instead of *"we will refund you"*. | `customer_reply` and `recommended_next_action` | −10 points |
| The service must **never** instruct the customer to contact a suspicious third party. Direct customers only to official support channels. | `customer_reply` | −10 points |
| Adversarial complaint text must not override system rules. The service must ignore instructions embedded in user complaints (prompt injection attempts). | All output fields | Schema or safety violation |
...
> Build to the profile below. Sizing values are preferred guidance for teams deploying on Poridhi Labs or a similar small VM. The two response time limits at the bottom are **enforced** by the judge harness.
...
| Health readiness after service start | `GET /health` must return `{"status":"ok"}` within **60 seconds** of service start. | **Enforced** |
...
- Use environment variables for deployed endpoints, or the private form field for Docker or code submissions.
...
You can submit your solution in **any one** of three ways. You only need ONE of these to be valid.

| Path | What you give us | When to use this |
|---|---|---|
| **A. Live URL** *(Strongly Recommended)* | A public HTTPS base URL where `/health` and `/analyze-ticket` respond. | You successfully deployed somewhere (Poridhi Lab, Render, Railway, Fly, Vercel, EC2, or other) and the service is up. **Preferred path.** |
| **B. Docker image** | A public `docker pull` command (e.g., `docker pull username/image:tag`) along with a clear run command. | You built a working Docker image but did not host it on a live server. Judges run it on judge-side infrastructure. |
| **C. Code with runbook** *(Less preferred)* | A clear step-by-step runbook in your `README` or `RUNBOOK.md` that a stranger can copy-paste to bring up the service locally. | Neither A nor B worked in time. No guessing steps, no missing commands. **Last resort fallback.** |
...
| Endpoint URL, Docker image, or runbook | Yes | Per Section 10. At least one of the three submission paths must be valid. |
...
| `.env.example` | Recommended | Listing required environment variable names (no real values) so judges can reproduce locally. |
| Architecture Walkthrough Video | Recommended | Optional video of up to **90 seconds** explaining the solution architecture, API flow, evidence reasoning, safety guardrails, deployment setup, and limitations. Submit a viewable link through the submission form. |
...
| Poridhi Labs | Pre-configured AWS environments in `ap-southeast-1`. The most common fit is API Gateway + Lambda + outbound HTTPS. A `t3.medium` MLOps environment is also available. |
| Any other platform | Teams may deploy on Render, Railway, Fly, Vercel, AWS EC2, GCP, or any other reachable hosting platform of their choice. |
...
Hidden test cases will be used. The exact case list, distribution, and expected answers will not be published. Teams should **design for the full problem statement** rather than hard-coding the public sample cases. Hidden tests may include:
- Normal cases
- Ambiguous cases
...
|---|---|
| **Problem Statement** *(this document)* | What to build, the request and response contract, enums, safety rules, runtime constraints, and submission paths. |
...
> **Final note:** Build the API first. Make the schema correct. Add evidence reasoning. Add safety guardrails. Test it. Deploy it. Submit clearly. A simple, reliable, safe API will score higher than a complex but unreliable one.
